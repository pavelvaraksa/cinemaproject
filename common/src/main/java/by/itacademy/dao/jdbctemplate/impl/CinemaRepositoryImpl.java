...
public class CinemaRepositoryImpl implements CinemaRepository {
    ...
    private Cinema getCinemaRowMapper(ResultSet rs, int i) throws SQLException {
        ...
        cinema.setAddress(rs.getString(CinemaColumn.ADDRESS));
    ...
    public Cinema save(Cinema cinema) {
        final String createQuery = "insert into m_cinema (name, address, phone_number, payment_method, created, changed, location_id, movie_id) " +
                "values (:name, :address, :phoneNumber, :paymentMethod, :created, :changed, :locationId, :movieId);";

        KeyHolder keyHolder = new GeneratedKeyHolder();

        MapSqlParameterSource params = new MapSqlParameterSource();
        params.addValue("name", cinema.getName());
        params.addValue("address", cinema.getAddress());
